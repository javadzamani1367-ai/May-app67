...
internal fun QuickAddSheet(
    ...
    val highlighter = remember(state.preview?.highlights, colors) {
        HighlightTransformation(state.preview?.highlights.orEmpty()) { kind ->
            when (kind) {
                SpanKind.TIME -> SpanStyle(background = colors.primaryContainer, color = colors.onPrimaryContainer)
                SpanKind.RECURRENCE -> SpanStyle(background = colors.tertiaryContainer, color = colors.onTertiaryContainer)
                SpanKind.DURATION -> SpanStyle(background = colors.secondaryContainer, color = colors.onSecondaryContainer)
            }
        }
    }
...
private class HighlightTransformation(
    ...
    private val styleOf: (SpanKind) -> SpanStyle,
